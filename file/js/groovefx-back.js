...
function the_awe_des_order(id,key,url_cible,url_wtp,color)  {	
	
	var url_fin;
	 
	if (url_wtp!='') {
		url_fin = url_cible + '?id='+id+'&key='+key+'&url='+url_wtp+'&color='+color;		
	} else {
		url_fin = url_cible + '/index.php?awesome_designer_back_popup=order&id='+id+'&key='+key+'&color='+color;
	}
	...
	if (!Modernizr.cssvwunit){				
		var wis = dw_getWindowDims();	
		jQuery(document.body).append(before+'<div id="div_groo" name="div_groo" class="awesome_designer_well_ss_ww" style="background-color:white;height:'+wis.height+'px;width:'+wis.width+'px;opacity: 1; visibility: visible; display: inline-block; outline: none;  text-align: left; position: relative; vertical-align: middle;"  ><iframe src="'+url_fin+'" id="awesome" name="awesome" style="border:0;max-width:1200px;padding:0px;margin:0px;height:'+wis.height+'px;width:'+wis.width+'px;"></iframe></div>'+after);
		window.onresize = the_awe_des_resize_iframe;
	} else {				
		jQuery(document.body).append(before+'<div id="div_groo" name="div_groo" class="awesome_designer_well" style="background-color:white;opacity: 1; visibility: visible; display: inline-block; outline: none;  text-align: left; position: relative; vertical-align: middle;" ><iframe src="'+url_fin+'" id="awesome" name="awesome" style="border:0;max-width:1200px;padding:0px;margin:0px;height:100vh;width:100vw;"></iframe></div>'+after);
	}
	...
	var messageEvent = eventMethod == "attachEvent" ? "onmessage" : "message";
	eventer(messageEvent, function (e) {		
		if (e.origin == url_cible.substring(0,e.origin.length)) {				
			message = e.data;
			jQuery('#div_groo_cont').remove(); 	
...
function add_color(col,val){

	if (val!='') {		
		color = jQuery("#groovefx_colors_"+val);	
	} else {
		color = $("#groovefx_colors");	
	}
...
function valid_color(val){	
	
	
	if (val===0 || val!='' ) {
		//alert(val);
		div_color = jQuery('#list_colors_'+val);
		color = jQuery(".css-groo-color-"+val);	
	} else {
	
		div_color = jQuery('#list_colors');
		color = jQuery("#groovefx_colors");			
	}
	var texte_col = color.val();
	...
	if (texte_col!='') {
		...
		for (var i = 0; i < col_ar.length; i++) {
			...
			if (checkHex(hexa)){
				...
				div_color.append('<div  style="display:inline-block;width:15px;height:15px;border:1px solid black;background-color:#'+hexa+';margin-left:5px;"></div>');
